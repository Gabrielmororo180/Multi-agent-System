...
for file_name in cluster_files:
    with open(os.path.join(cluster_folder, file_name), 'r', encoding='latin-1') as f:
        print(f"Processando o arquivo de cluster: {file_name}")
        cluster = []
        for line in f:
...
for file_name in seq_files:
    with open(os.path.join(cluster_folder, file_name), 'r') as f:
        print(f"Processando o arquivo de sequência: {file_name}")
        seq = []
        for line in f:
            elements = line.strip().split(',')
            converted_elements = []
